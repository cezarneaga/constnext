...
export async function fetchGraphQL(
  ...
  if (json.errors) {
    console.warn(
      ...
      json.errors.map((m: { message: string }) => m.message),
    );
  }
...
export function extractProject(fetchResponse: GraphQLResponse): Project | null {
  return fetchResponse?.data?.projectCollection?.items?.[0] || null;
}

export function extractProjectEntries(fetchResponse: GraphQLResponse): Project[] {
